# GrapeRank Algorithm: Three Versions Explained
...
3. [Version 1 - Stateless Iterative](#version-1--stateless-iterative-v1easy)
4. [Version 2 - Stateful Full Recomputation](#version-2--stateful-full-recomputation-v2stateful)
5. [Version 3 - Forward Propagation](#version-3--forward-propagation-v3recursive)
...
## Version 1 -- Stateless Iterative (`v1Easy`)

> **File:** `v1Easy/GrapeRank.kt` (68 lines)
...
## Version 3 -- Forward Propagation (`v3Recursive`)

> **File:** `v3Recursive/RecursiveGrapeRank.kt` (162 lines)
...
```
                    V1 Easy          V2 Stateful       V3 Recursive
                ─────────────    ─────────────────   ──────────────────
...
1. **V1** is a **textbook implementation** -- a pure function, easy to reason about, perfect for understanding the math.
2. **V2** adds **statefulness and reactivity** -- scores are stored and automatically recomputed on mutations, but the inner loop is still brute-force over the entire graph.
3. **V3** adds **surgical precision** -- by indexing outgoing edges and using BFS propagation, it only touches nodes whose scores actually need to change, making it dramatically more efficient for large, dynamic social graphs.
